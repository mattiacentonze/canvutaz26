...
let backendError = '';
let backendSource = '';
const DATA_SHEETS = ['giorni','cose_da_fare','alloggi','costi','auto_documenti','prenotazioni','link_utili','note'];
const EMPTY_STATE = {
  meta: {
...
function createEmptyState(){ return structuredClone(EMPTY_STATE); }
function readCache(){ try{ const raw = localStorage.getItem(STORAGE_KEY); if(!raw) return null; const parsed = JSON.parse(raw); if(parsed && typeof parsed === 'object' && 'data' in parsed) return parsed; return { signature: dataSignature(parsed), data: parsed }; } catch(e){ return null; } }
function clearCache(){ try{ localStorage.removeItem(STORAGE_KEY); } catch(e){} }
function writeCache(data){ try{ localStorage.setItem(STORAGE_KEY, JSON.stringify({ signature: dataSignature(data), data })); } catch(e){} }
function dataSignature(data){ try{ return JSON.stringify(data || null); } catch(e){ return String(Date.now()); } }
function applyRemoteData(data, cached){ const currentSignature = cached?.signature || ''; const remoteSignature = dataSignature(data); if(currentSignature && currentSignature !== remoteSignature){ clearCache(); } state = data; writeCache(state); }
...
async function loadGoogleSheetData(){ const id = sheetIdFromUrl(CONFIG.GOOGLE_SHEET_URL); if(!id) throw new Error('GOOGLE_SHEET_URL mancante o non valido'); const entries = await Promise.all(DATA_SHEETS.map(async name => [name, await loadGoogleSheetRows(id, name)])); return { ...createEmptyState(), ...Object.fromEntries(entries) }; }
function sheetIdFromUrl(url=''){ const m = String(url).match(/\/spreadsheets\/d\/([^/]+)/); return m ? m[1] : ''; }
function loadGoogleSheetRows(id, sheetName){ return new Promise((resolve,reject)=>{ const cb = `usa26_sheet_${Date.now()}_${Math.random().toString(36).slice(2)}`; const u = new URL(`https://docs.google.com/spreadsheets/d/${id}/gviz/tq`); u.searchParams.set('sheet', sheetName); u.searchParams.set('tqx', `out:json;responseHandler:${cb}`); const s = document.createElement('script'); const t = setTimeout(()=>{ cleanup(); reject(new Error(`Timeout Google Sheet ${sheetName}`)); }, 9000); function cleanup(){ clearTimeout(t); delete window[cb]; s.remove(); } window[cb] = (response)=>{ cleanup(); if(!response || response.status !== 'ok') { reject(new Error(`Risposta Google Sheet non valida per ${sheetName}`)); return; } resolve(tableToRows(response.table)); }; s.onerror = ()=>{ cleanup(); reject(new Error(`Errore caricamento Google Sheet ${sheetName}`)); }; s.src = u.toString(); document.body.appendChild(s); }); }
function tableToRows(table){ const rows = table?.rows || []; if(rows.length < 2) return []; const values = rows.map(r => (r.c || []).map(cell => cell?.f ?? cell?.v ?? '')); const headers = values[0].map(h => String(h || '').trim()); return values.slice(1).filter(row => row.some(cell => String(cell || '').trim() !== '')).map(row => { const obj = {}; headers.forEach((h, i) => { if(h) obj[h] = row[i] || ''; }); return obj; }); }
